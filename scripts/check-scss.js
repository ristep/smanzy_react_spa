...
try {
  ...
  // Rewrite alias imports in .scss files
  const scssFiles = [];
  ...
  // Run Sass CLI via npx (uses local devDependency if present)
  console.log('Running: npx sass --no-source-map', `${tmpDir}:${outDir}`);
  const res = spawnSync('npx', ['sass', '--no-source-map', `${tmpDir}:${outDir}`], { stdio: 'inherit' });
